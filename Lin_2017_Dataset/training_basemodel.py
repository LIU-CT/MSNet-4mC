...
########## testing data preparing ##########
    ...
    modeltest = model(load_pretrain = False )
    ...
    best_acc = 0
    save_path = '00_results/'

########## Loss ##########
    ### with class weights
    class_weights = torch.tensor(np.load('class_weight.npy'))#tensor([1.2868, 1.5283, 1.3985, 2.9160, 2.5329, 2.0683], dtype=torch.float64)
    class_weights = class_weights.shape[0] * class_weights/class_weights.sum()#tensor([0.6582, 0.7817, 0.7153, 1.4915, 1.2955, 1.0579], dtype=torch.float64)
    loss_fn = CE_Loss(class_weights=class_weights).cuda()
    
    loss_fn = loss_fn.to(device)
########## Optimizer ##########
    learning_rate = 5e-02 #2.5e-02 
...
########## Epochs ##########
    ...
    scheduler = LR_Scheduler('cos', learning_rate, epochs, len(trainloader), warmup_epochs=warm)####cos学习率###
    ...
    for epoch in range(epochs):
        ...
        if acc > best_acc:
            ...
            torch.save(modeltest.state_dict(), save_path + 'Model_basemodel.pth')    
