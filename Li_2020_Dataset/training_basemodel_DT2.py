# -*- coding: utf-8 -*-
"""
Created on Sun May  8 20:45:10 2022
...
########## base model ##########
ls_txt_name =['basemodel']

txt_ALL_train = txt_A_train + txt_C_train +txt_D_train + txt_E_train + txt_GP_train + txt_GS_train
data_txt_train_ls = [txt_ALL_train]
...
for num in range(len(data_txt_train_ls)):
    data_txt_train = data_txt_train_ls[num]
    data_txt_test = data_txt_test_ls[num]

########## sequence & target ##########
    df_fastas_train = get_df_fastas_DT2(data_txt_train)
    ...
    train_X_encodings = BINARY_w_labels_DT2(sequence_train)
...
########## training data preparing ##########
    ...
    trainloader = Data.DataLoader(train_dataset, batch_size=512, shuffle = True, num_workers=0) ##DT2 default:512     
########## testing data preparing ##########
    ...
    testloader = Data.DataLoader(test_dataset, batch_size=256, shuffle = True, num_workers=0) ##DT2 default:256 
    ...
    best_acc = 0
    save_path = '00_results/' #save path
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    modeltest = model(load_pretrain = load_pretrain, load_path = load_path )
    # base model:load_pretrain = False
    # fine-tuning:load_pretrain = True
    modeltest = modeltest.to(device)
########## Loss ##########
    ### with class weights
    class_weights = torch.tensor(np.load('class_weight.npy'))#tensor([1.0915, 1.2255, 1.2576, 4.5828, 3.7388, 2.9500], dtype=torch.float64)
    class_weights = class_weights.shape[0] * class_weights/class_weights.sum()# tensor([0.4411, 0.4953, 0.5082, 1.8521, 1.5110, 1.1922], dtype=torch.float64)
    loss_fn = CE_Loss(class_weights=class_weights).cuda()
    
    loss_fn = loss_fn.to(device)
########## Optimizer ##########
    learning_rate = 2.5e-02 
    optimizer = optim.SGD(modeltest.parameters(), lr=learning_rate, momentum=0.9, weight_decay=5e-04) # 5e-04    
########## Epochs ##########   
    total_train_step = 0
    total_test_step = 0
    ...
    scheduler = LR_Scheduler('cos', learning_rate, epochs, len(trainloader), warmup_epochs=warm) ### Cos learning schedual ###    
    ...
    for epoch in range(epochs):            
        ...
        for batch_train in trainloader: 
            ...
            loss = loss_fn(outputs, batch_y, sign_X) ### with class weights            
        ...
        print("ACC on the training set: {}".format(total_accuracy_train/train_data_size))
