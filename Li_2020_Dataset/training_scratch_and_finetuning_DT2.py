...
########## sequence & target ##########
    ...
    for tem in range(len(train_X_encodings)):
        if len(train_X_encodings[tem])!=41:
            print(tem)
...
########## test data preparing ##########
    ...
    modeltest = model(load_pretrain = load_pretrain, load_path = load_path, drop_s = 0.25 )
    ####base model:load_pretrain = False   
    ####fine-tuning:load_pretrain = True
    ####scratch:load_pretrain = False
